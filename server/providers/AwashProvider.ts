...
export class AwashProvider extends BaseReceiptProvider {
  readonly bankName = "Awash Bank";
  readonly bankCode = "Awash";
  readonly supportedDomains = ["awashbank.com", "awashpay.awashbank.com"];
  ...
  canHandle(input: string, options?: ProviderOptions): boolean {
    ...
    if (/^AW[A-Z0-9]{5,14}$/i.test(clean) || /^AWASH[A-Z0-9]{5,12}$/i.test(clean)) {
      return true;
    }
  ...
  buildReceiptUrl(input: string, options?: ProviderOptions): string {
    ...
    return `https://awashpay.awashbank.com:8225/-${reference}`;
  }
  ...
  parseReceipt(content: string, url: string, options?: ProviderOptions): NormalizedReceiptData | null {
    ...
    try {
      ...
      if (
        ...
        fullText.toLowerCase().includes("invalid token") ||
        fullText.toLowerCase().includes("no record")
      ) {
        return null;
      }
      ...
      $("tr, div, p, span").each((_, el) => {
        ...
        if (lower.includes("receiver") || lower.includes("payee") || lower.includes("to")) {
          const val = text.split(/[:\-]/)[1]?.trim();
      ...
      return {
        verified: true,
        bank: this.bankCode,
        transaction_id: transactionId || "AWASH_REF",
        payer: this.cleanString(payer) || "Awash Bank Customer",
        ...
        reference: transactionId || "AWASH_REF",
        receipt_url: url
      };
    } catch (err: any) {
      console.error("Error parsing Awash receipt:", err);
      return null;
    }
  }
}
